...
template<typename T0, typename T1> struct ImplicitTreap {
 ...
 public:
  ...
  // 先頭から pos の位置 (pos == 0 なら先頭)に x を挿入 O(log N)
  void insert(int pos, T0 x) { insert(root, pos, new Node(x, rnd.random(), u0, u1)); }
  // [l,r) に x を作用　O(log N)
  void update(int l, int r, T1 x) { update(root, l, r, x); }
  // [l,r) の累積 O(log N)
  T0 prod(int l, int r) { return prod(root, l, r); }
  // k∈[l, r) で f0(tr[k], x) != x となる最左/右のもの。存在しない場合は-1 O(log N)
  ...
  // [l,r) の先頭が m になるよう回転 O(log N)
  void rotate(int l, int m, int r) { rotate(root, l, m, r); }
  // 配列の中身を print (for debug)
  void dump() {
    dump(root);
    cout << endl;
  }
  // O(log N)
  T0 operator[](int pos) { return prod(pos, pos + 1); }
};
