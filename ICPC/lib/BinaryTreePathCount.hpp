...
template<typename T> vector<T> BinaryTreePathCount(ll n) {
  ll D = 63 - __builtin_clzll(n + 1), L = n - (1LL << D) + 1;
  vector<T> cnt(130), bin(130, 1);
  for(ll i = 1; i < 130; i++) { bin[i] = bin[i - 1] * 2; }
  map<ll, T> memo[65][130];
  for(ll k = 0; k < 130; k++) {
    auto calc = [&](auto &&calc, ll d, ll l) -> T {
      if(!d) { return 0; }
