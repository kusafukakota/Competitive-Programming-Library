template<typename T> struct Sum2D {
  ...
  void imos_add(int lx, int ly, int rx, int ry, T a = 1) { add(lx, ly, a), add(lx, ry, -a), add(rx, ly, -a), add(rx, ry, a); }
  void build() {
    for(int i = 1; i < (int)v.size(); i++) {
      for(int j = 1; j < (int)v[i].size(); j++) { v[i][j] += v[i][j - 1] + v[i - 1][j] - v[i - 1][j - 1]; }
    }
  }
  T sum(int lx, int ly, int rx, int ry) const { return v[rx][ry] - v[lx][ry] - v[rx][ly] + v[lx][ly]; }
  T operator()(int x, int y) const { return v[x][y]; }
  T operator()(int lx, int ly, int rx, int ry) const { return v[rx][ry] - v[lx][ry] - v[rx][ly] + v[lx][ly]; }
